...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <div className="group bg-white rounded-2xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.05)] hover:shadow-[0_10px_30px_rgba(0,0,0,0.1)] transition-all duration-500 border border-gray-100 flex flex-col h-full relative">
            <Link href={`/product/${product.id}`} className="relative h-[250px] overflow-hidden block">
                <Image
                    src={product.image}
                    alt={product.name}
                    fill
            ...
            <div className="p-4 flex-grow flex flex-col">
                ...
                <div className="flex items-baseline gap-2 mb-5">
                    <span className="text-brand-plum font-black text-xl">{product.price}</span>
                    <span className="text-gray-300 line-through text-xs font-bold">{product.oldPrice}</span>
                </div>

                {/* Refined Actions - Optimized for Sales */}
                <div className="mt-auto space-y-2">
                    <div className="flex gap-2">
                        <button
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); trackAddToCart(product); addToCart(product); }}
